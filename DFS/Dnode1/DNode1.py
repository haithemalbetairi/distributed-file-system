...
global IP,PORT,PATH
IP,PORT,PATH = "127.0.0.1",8888,Path.home()/'DFS/Dnode1'
...
if __name__ == "__main__":
    ...
    # this opens the ftpserver2.py file in a new console (shell=True) so that we can view the FTP logs
    if(platform.system()=='Windows'):
        print("I'm a windows node!")
        subprocess.call("start cmd /K python ftpserver1.py", shell=subprocess) 

    elif(platform.system()=='Linux'):
        print("I'm a Linux node!")
        subprocess.Popen(["python3", "ftpserver1.py"], stdout=subprocess.PIPE)

    else:
        print("I'm an else node!")
        subprocess.Popen(["python3", "ftpserver1.py"], stdout=subprocess.PIPE)
        
    # Start the DNode server
    t1.start()                                  

""" NOTE:
Here we call the ftpserver script separately as including the FTP script in this file 
